        ...
        if(player !== null){
            if(typeof player.game !== 'undefined'){
                switch(player.game.id){
                    case 'cah':
                        gameHandler = new CahHandler(player, app, socket);
                        break;
            ...
            if( sessionStorage ){
                ...
                sessionStorage.setItem('cahPlayer', JSON.stringify(storedPlayer));
            }
        }
    };
...
let showChatMessage = function(message, playerName) {
    ...
    if( chatBox ){
        chatBox.innerHTML += `
            <p>
                <span class="text-muted">${playerName}:</span>
                ${message}
            </p>
        `;
...
`;
    ...
    document.getElementById('choose_button').addEventListener('click', (event) => {
        ...
        socket.emit('choose-game', nameInput.value, 'cah');
    });

    if( sessionStorage ){
        let existingPlayer = sessionStorage.getItem('cahPlayer'),
            removeStoredPlayer = () => sessionStorage.removeItem('cahPlayer');
