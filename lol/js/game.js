        ...
        if(player !== null){
            if(typeof player.game !== 'undefined'){
                switch(player.game.id){
                    case 'lol':
                        gameHandler = new LolHandler(player, app, socket);
                        break;
            ...
            if( sessionStorage ){
                ...
                sessionStorage.setItem('lolPlayer', JSON.stringify(storedPlayer));
            }
        }
    };
...
`;
    ...
    document.getElementById('choose_button').addEventListener('click', (event) => {
        ...
        socket.emit('choose-game', nameInput.value, 'lol');
    });

    if( sessionStorage ){
        let existingPlayer = sessionStorage.getItem('lolPlayer'),
            removeStoredPlayer = () => sessionStorage.removeItem('lolPlayer');
